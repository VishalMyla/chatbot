...
> 👍NoteFor detailed information about our APIs and for hands-on testing, refer to the documentation inAPI Overviewand follow the step-by-step guide on making your first API call inMake Your First API Call.WhenDual Eligibilityis enabled, a reward can be issued to multiple users registered with the same secondary identifiers, as long as they have unique primary identifiers.

## 👍Note

Note

- For detailed information about our APIs and for hands-on testing, refer to the documentation inAPI Overviewand follow the step-by-step guide on making your first API call inMake Your First API Call.

For detailed information about our APIs and for hands-on testing, refer to the documentation inAPI Overviewand follow the step-by-step guide on making your first API call inMake Your First API Call.

- WhenDual Eligibilityis enabled, a reward can be issued to multiple users registered with the same secondary identifiers, as long as they have unique primary identifiers.
...
> 📘NoteIf rewards are created for a customer segment, they are displayed to the user only if the user is part of the segment or partition mapping associated with the reward.
...
| status | Contains the status details of the operation, including whether it succeeded, a code for reference, and a message. |

| success | Indicates whether the operation was successful (truefor success andfalsefor failure). |

| code | A numerical code representing the status of the operation (e.g.,200for success). |

| message | A brief description or message about the status of the operation. |

| reward | Provides detailed information about the reward, including metadata, restrictions, and configurations. |

| id | A unique identifier assigned to the reward to differentiate it from others. |

| name | The name of the reward as displayed in the system or user interface. |

| description | A short text explaining what the reward is about or any additional details about it. |

| imageId | An identifier linking to the reward's primary image stored in the system. |

| imageUrl | A web URL pointing to the full-sized image of the reward. |

| thumbnailId | An identifier for the thumbnail version of the reward image. |

| thumbnailUrl | A web URL pointing to the thumbnail-sized image of the reward. |

| termAndConditionsId | An identifier linking to the terms and conditions document for the reward. |

| termAndConditionsUrl | A web URL pointing to the terms and conditions document for the reward. |

| tier | Specifies the tier level (if applicable) to which the reward belongs, typically used in loyalty programs. |

| label | A tag or label used to categorize or further describe the reward. |

| priority | The priority level of the reward; higher priority rewards might appear earlier in listings. |

| intouchPoints | The number of loyalty points required to redeem the reward. |

| group | Defines a grouping attribute for the reward if applicable. |

| startTime | The timestamp when the reward becomes available for redemption. |

| endTime | The timestamp when the reward is no longer available for redemption. |

| expired | Indicates whether the reward has expired and can no longer be redeemed. |

| started | Indicates whether the reward is active and can be redeemed. |

| programId | A unique identifier linking the reward to a specific loyalty or promotional program. |

| categoryList | Lists the categories to which the reward belongs, often used for filtering or display purposes. |

| id | A unique identifier for a specific category associated with the reward. |

| name | The name of the category (e.g., "electronics" or "clothing"). |

| enabled | Indicates whether the category is active and available for use. |

| customFields | Contains additional metadata or attributes related to the reward, defined as key-value pairs. |

| testdate | An example custom field in date format; its purpose may vary based on implementation. |

| CF1 | An example custom text field; its purpose may vary based on implementation. |

| loyaltyProgramCriteria | Specifies any criteria related to loyalty programs that impact this reward. |

| rewardRestrictions | Details about any restrictions or limitations that apply to the reward. |

| isValid | Indicates whether the defined restrictions are valid and enforceable. |

| customerLevel | Lists constraints specific to customer-level restrictions, such as usage limits and frequency types. |

| id | A unique identifier for a customer-level restriction. |

| isValid | Indicates if the customer-level restriction is valid for enforcement. |

| consumed | Tracks the quantity or value consumed by the customer under this restriction. |

| repeatFrequencyType | Specifies the frequency type for the restriction (e.g., "DAYS", "MONTHS", "NO_LIMIT"). |

| limit | The maximum allowable value under this restriction (e.g., maximum redemptions). |

| interval | Defines the duration or period (e.g., in days or months) for which the restriction applies. |

| kpi | A key performance indicator related to the restriction (e.g., "QUANTITY" or "REDEMPTION_VALUE"). |

| rewardLevel | Constraints applicable at the reward level, such as global limits on redemptions. |

| id | A unique identifier for a reward-level restriction. |

| isValid | Indicates if the reward-level restriction is valid and enforceable. |

| consumed | It refers to the total amount of Redemption Value or Quantity that have been used or redeemed. |

| repeatFrequencyType | The type of frequency for the restriction at the reward level (e.g., "NO_LIMIT"). |

| limit | The maximum value allowed under the reward-level restriction. |

| kpi | A key performance indicator related to the reward-level restriction. |

| appliedPromotions | Lists any promotions currently applied to the reward. |

| groups | Lists any specific groups to which the reward belongs, often for segmentation purposes. |
...
| paymentConfigs | Configuration details for different payment methods associated with the reward. |

| paymentMode | The type of payment mode allowed for this reward (e.g., POINTS, CASH, POINTS_CASH). |

| id | A unique identifier for the payment configuration. |

| conversionRatio | The conversion ratio used for points or cash redemption for this reward. |

| orgLevelRestrictions | Organization-level restrictions that apply to the reward. |

| isValid | Indicates whether the organization-level restrictions are valid and applicable. |
